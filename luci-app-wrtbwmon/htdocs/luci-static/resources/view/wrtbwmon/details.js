...
function resolvefromDHCP() {
	return uci.load('wrtbwmon').then(function() {
		return L.resolveDefault(uci.get_first('wrtbwmon', 'wrtbwmon', 'resolve_hostnames'), 0);
	}).then(function(res) {
		uci.unload('wrtbwmon');
		return res;
	});
...
function resolveCustomizedHostName() {
	...
	return resolvefromDHCP().then(function(readDHCPhostnames) {
		return resolveDHCPfile().then(function (dhcpHostnames) {
			return L.resolveDefault(fs.trimmed(hostNameFile), []).then(function(hostNames) {
				if (readDHCPhostnames) {
					hostNames += "\r\n" + dhcpHostnames.stdout;
				}
...
function resolveDHCPfile() {
	return getPath().then(function(res) {
		var data = renameFile(res, 'hosts');
		return data;
	}).then(function(DHCPhostfilename) {
		return fs.exec('/bin/cat', [DHCPhostfilename]);
	})
}
